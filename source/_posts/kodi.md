---
title: rclone挂载网盘 + OverlayFS + tmm + kodi打造家庭媒体中心
...
- kodi
- 多媒体
index_img: "/images/thumbs/IMG_9433.webp"
---

之前我的家庭媒体中心一直是使用本地 nas 存高清片源 + tmm（TinyMediaManager）做媒体搜刮器 + 电视上安装 kodi 做播放器的方案，但是这种方式的缺点是总得自己花时间下载片源。后来用了一段时间别人的 emby 服务，然鹅现在也用不了了。索性自己用 rclone 挂载网盘来管理片源，于是这周末折腾了下媒体管理中心，踩坑各种不同的方案之后选择了一个网上不太常见但个人感觉很好用的方案，于是记录一下。
...
一开始尝试了 emby ，搜刮直接花了一个白天的时间，然后发现海报怎么都加载不出来，另外很多视频都会走转码（我就很奇怪为啥，我电视端明明支持这种格式，服务端仍然会转码），emby 的硬件解码需要付费开启，遂放弃。然后尝试了 jellyfin，这个转码倒是比较完美的支持硬件解码了，然鹅搜刮还是难用，一是慢，二是不好进行微调。最后还是用回了 kodi。下面是 kodi 方案的教程。
...
另外搜刮有个小技巧，如果你的文件名也是和我一样是中文+英文混合命名的，并且包含什么 `x264` `web-dl` 之类的标签，那搜挂起大概率会被这些词干扰而什么都搜刮不到。所以 tmm 在搜挂时支持从文件名中剔除掉一些字符，而且支持正则表达式。我直接把 `[a-z]*` 作为敏感词，可以提出掉所有的英文字符，只用中文来搜刮，这样准确率高很多。
...
启动后进入到容器里，`vi /etc/cont-init.d/10-cjk-font.sh` 编辑这个文件，把 http://dl-cdn.alpinelinux.org 替换为 http://mirrors.tuna.tsinghua.edu.cn/ 这样下载中文字体会快很多。然后重启容器。（如果你的服务器本身能正常访问这些域名，则不需要这一步，上面命令里的 `--add-host` 也可以删去）
